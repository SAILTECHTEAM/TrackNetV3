...
import cv2
import time
from tracknet_infer import TrackNetInfer, TrackNetConfig, TrackNetModule, InpaintModule
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if inpaint_mod is None and infer.inpaintnet is not None and track_mod.img_scaler is not None:
    ...
    print(f"[INFO] Video processing time: {end - start:.3f}s for {fid} frames ({fid / (end - start):.2f} FPS)")

    results.extend(track_mod.flush())
    if inpaint_mod is not None:
        results.extend(inpaint_mod.flush())

    print(f"[DONE] total_frames={fid}, outputs={len(results)} (bg_mode={infer.bg_mode}, median_warmup={median_warmup})")
    ...
    print(f"  preprocess: {s['t_preprocess']:.3f}s  ({s['t_preprocess']/max(1,s['push_calls']):.6f}s/call)")
    print(f"  forward:    {s['t_forward']:.3f}s  ({s['t_forward']/max(1,s['push_calls']):.6f}s/call)")
    ...
    if inpaint_mod is not None:
        ...
        print(f"  forward:    {si['t_forward']:.3f}s  ({si['t_forward']/max(1,si['push_calls']):.6f}s/call)")
        print(f"  post:       {si['t_post']:.3f}s  ({si['t_post']/max(1,si['push_calls']):.6f}s/call)")
