...
if not os.path.exists(os.path.join(data_dir, "val")):
    ...
    for match_dir in match_dirs:
        ...
        shutil.move(
            os.path.join(data_dir, "train", match_dir, "csv", f"{rally_id}_ball.csv"),
            os.path.join(data_dir, "val", match_dir, "csv", f"{rally_id}_ball.csv"),
        )
        shutil.move(
            os.path.join(data_dir, "train", match_dir, "video", f"{rally_id}.mp4"),
            os.path.join(data_dir, "val", match_dir, "video", f"{rally_id}.mp4"),
        )
        shutil.move(
            os.path.join(data_dir, "train", match_dir, "frame", rally_id),
            os.path.join(data_dir, "val", match_dir, "frame", rally_id),
        )
        shutil.copy(
            os.path.join(data_dir, "train", match_dir, "median.npz"),
